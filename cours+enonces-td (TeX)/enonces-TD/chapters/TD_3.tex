...
Calculer la date de Pâques est loin d'être une chose facile. Il existe plusieurs méthodes, 
dont la suivante connue
sous le nom d'algorithme de Oudin qui a l'avantage de
demander peu d'opérations. Il en existe une version généraliste (sans
limite de siècle), mais nous présentons ici sa forme simplifiée valable pour le
...
Dans la présentation de l'algorithme ci-dessous, chaque ligne introduit une nouvelle variable qui dépend 
des précédentes. Les divisions doivent toujours être entières. L'exemple en bleu indiquent les calculs pour l'année 2007.
\medskip

\begin{enumerate}[label=\arabic*.]
\item G qui représente l'écart d'or diminué de 1 : diviser l'année par 19, en prendre le reste ;  
...
\item I représente le nombre de jours entre la pleine lune pascale et le 21 mars : (K × P × Q-1)×K +H ; 
...
\item R le résultat final, enfin 28 + I - J2. 
...
\begin{enumerate}
\item Calculez la date de Paques pour l'année 2007 en suivant la méthode de Oudin.
\item Debugguez au fûr et à mesure les valeurs des variables avec la fonction 
print pour vérifier votre implémentation de l'algorithme à l'aide de l'exemple bleu.
\
\begin{lstlisting}
 G = ...
 C = ...
...
\begin{enumerate}
\item Modifier le script pour mettre le calcul de Pâques
...
\item Réaliser un programme qui demande une année à l'utilisateur dans le programme
principal, puis affiche la date de Paques pour cette année.
...
\item \'Ecrire toutes les dates de Paques du XXIe siècle dans un fichier texte.
\
\begin{lstlisting}
